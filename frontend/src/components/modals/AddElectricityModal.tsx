...
export const AddElectricityModal: React.FC<AddElectricityModalProps> = ({
  ...
  const effectiveRate = myUnitsVal > 0 ? billVal / myUnitsVal : 0;
      ...
      <div className="relative w-full max-w-lg bg-zinc-950 border border-zinc-800 rounded-2xl p-4 sm:p-6 shadow-2xl space-y-4 sm:space-y-5 max-h-[90dvh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
              <label className="block text-xs font-mono text-zinc-400 mb-1">Payment Date</label>
              <input
                type="date"
                value={paidDate}
          ...
          <div className="p-3.5 rounded-xl bg-cyan-950/20 border border-cyan-800/30 space-y-1.5 text-xs font-mono">
            ...
            <div className="flex justify-between text-zinc-300">
              ...
              <span className="font-bold text-white tabular-nums">₹{billVal.toFixed(2)}</span>
            </div>
